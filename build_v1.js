...
DataLoader.prototype = {
  ...
  loadItems: function(callback) {
    AjaxHandler.request('GET', 'build-api.php?q=items', function(status, responseText) { //justin changed
      if (status != 200) {
  ...
  loadTypes: function(callback) {
    AjaxHandler.request('GET', 'build-api.php?q=types', function(status, responseText) { //justin changed
      if (status != 200) {
...
ShipBuilder.prototype.addActive = function(item_name) {
  if (!this.activeItemList)
    //this.activeItemList = new ListData([], 'active-items', document.body, function(item) {
...
var PageHandler = {
  CONTAINER_ID: 'builder', //changed

  builder: null,

  handleLoad: function() {
    //PageHandler.builder = new ShipBuilder(document.body);
